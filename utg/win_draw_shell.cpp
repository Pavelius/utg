...
bool file_open_dialog(const char* title, char* path, const char* filter, int filter_index, const char* ext) {
	OPENFILENAME	ofn;
...
bool file_save_dialog(const char* title, char* path, const char* filter, int filter_index) {
	OPENFILENAME ofn;
	memset(&ofn, 0, sizeof(ofn));
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = GetActiveWindow();
	...
	if(result) {
		if(filter && !szext(path)) {
			...
			if(p1) {
				stringbuilder sb(path, path + 256);
				sb.set((char*)zend(path));
				sb.add(".");
				auto ps = (char*)zend(path);
				while(ischa(*p1))
					*ps++ = *p1++;
				*ps++ = 0;
			}
...
bool color_pick_dialog(struct color& result, struct color* custom) {
...
bool folder_choose_dialog(const char* title, char* path) {
	BROWSEINFOA binf = {0};
